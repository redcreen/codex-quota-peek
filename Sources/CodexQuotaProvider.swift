...
struct CodexQuotaTrendSummary {
    ...
    func chartPresentation(
        ...
        let days = zip(dailyUsageBars, hoursByDay).map { day, hours in
            ChartPresentation.Day(
                ...
                hours: hours,
                isAheadOfPace: showsPaceHighlights && day.isAheadOfDailyPace,
    ...
    func menuText(language: AppLanguage = .english, weeklyPacingMode: WeeklyPacingMode = .balanced56) -> String? {
        ...
        return DailyUsageChartRenderer.render(chart)
    }
...
extension CodexQuotaProvider {
    private static func dailyUsageBars(
        ...
        DailyUsageLedger.build(rows: rows).map {
            CodexQuotaTrendSummary.DailyUsageBar(
                date: $0.date,
                usedPercent: $0.usedPercent,
                cumulativeUsedPercent: $0.cumulativeUsedPercent,
                expectedUsedPercent: $0.expectedUsedPercent,
                expectedDailyUsedPercent: $0.expectedDailyUsedPercent,
                isFuture: $0.isFuture
            )
        }
    }
