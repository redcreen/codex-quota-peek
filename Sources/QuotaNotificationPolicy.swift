...
struct QuotaNotificationPreferences: Equatable {
    let lowQuotaEnabled: Bool
...
enum QuotaNotificationPolicy {
    ...
    static func nextEvent(
        ...
        preferences: QuotaNotificationPreferences = QuotaNotificationPreferences(
        ...
        if preferences.lowQuotaEnabled, current.sessionLevel == .critical, previousSession != .critical {
            return QuotaNotificationEvent(
                title: "5-hour quota is low",
                body: "Session remaining dropped below 30%."
            )
        }
        if preferences.lowQuotaEnabled, current.sessionLevel == .warning, previousSession == .none {
            return QuotaNotificationEvent(
                title: "5-hour quota warning",
                body: "Session remaining dropped below 50%."
            )
        }
        if preferences.lowQuotaEnabled, current.weeklyLevel == .critical, previousWeekly != .critical {
            return QuotaNotificationEvent(
                title: "7-day quota is low",
                body: "Weekly remaining dropped below 30%."
            )
        }
        if preferences.lowQuotaEnabled, current.weeklyLevel == .warning, previousWeekly == .none {
            return QuotaNotificationEvent(
                title: "7-day quota warning",
                body: "Weekly remaining dropped below 50%."
            )
        }
        if preferences.paceEnabled, current.paceLevel == .critical, previousPace != .critical {
            return QuotaNotificationEvent(
        ...
        if preferences.paceEnabled, current.paceLevel == .warning, previousPace == .none {
            return QuotaNotificationEvent(
                title: "Usage pace warning",
                body: presentation.paceMessage.map { "\($0)." } ?? "Quota usage is ahead of your selected pace."
            )
        }
        if preferences.resetEnabled, current.sessionResetSoon, !previousSessionResetSoon {
            return QuotaNotificationEvent(
                title: "5-hour window resets soon",
                body: "The 5-hour quota window resets in less than 15 minutes."
            )
        }
        if preferences.resetEnabled, current.weeklyResetSoon, !previousWeeklyResetSoon {
            return QuotaNotificationEvent(
